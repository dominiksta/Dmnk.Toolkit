...
public class ViewModelRegistryGenerator : IIncrementalGenerator
{
        ...
        SourceProductionContext context)
    {
        ...
        var source = GenerateRegistrationClass(distinctViewModels, compilation.AssemblyName ?? "Assembly");
        const string fileName = 
            "ViewModelRegistryExtensions_CHECK_ViewModelFor_ANNOTATIONS_IF_COMPILATION_FAILS.g.cs";
        context.AddSource(fileName, SourceText.From(source, Encoding.UTF8));
    }

    private static string GenerateRegistrationClass(List<ViewModelInfo> viewModels, string assemblyName)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("#nullable enable");
        sb.AppendLine();
        sb.AppendLine("namespace Dmnk.Blazor.Mvvm;");
        sb.AppendLine();
        sb.AppendLine("/// <summary>");
        sb.AppendLine("/// Auto-generated extension methods for registering ViewModels with their Views.");
        sb.AppendLine("/// </summary>");
        sb.AppendLine("public static class IViewModelRegistryExtensions");
        sb.AppendLine("{");
        sb.AppendLine("    /// <summary>");
        sb.AppendLine($"    /// Registers all ViewModels from {assemblyName} that are marked with [ViewModelFor] attribute.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    public static void AutoRegisterFromSourceGen(this IViewModelRegistry registry)");
        sb.AppendLine("    {");
